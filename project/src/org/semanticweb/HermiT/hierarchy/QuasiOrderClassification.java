...
public class QuasiOrderClassification {
    ...
    private ReADSetting m_reAD;
    private int STNumber;
    private boolean useTech;
    private boolean classificationFinished = false;
    
    public QuasiOrderClassification(Tableau tableau,ClassificationProgressMonitor progressMonitor,AtomicConcept topElement,AtomicConcept bottomElement,Set<AtomicConcept> elements, ReADSetting reAD) {
        m_tableau=tableau;
        ...
        Relation<AtomicConcept> relation=new Relation<AtomicConcept>() {
            public boolean doesSubsume(AtomicConcept parent,AtomicConcept child) {
                ...
                if (allKnownSubsumers.contains(parent))
                    {
            		long knowT = System.currentTimeMillis();
            		long time = knowT-currentCPUTime1;
            		//System.out.println("already know with " +time +" ms");
                	return true;}
                else if (!m_possibleSubsumptions.getSuccessors(child).contains(parent))
                    {
                	long dontknowT = System.currentTimeMillis();
            		long time = dontknowT-currentCPUTime1;
            		//System.out.println("already know with " +time +" ms");             	
                	return false;}
        		...
        		long stTime = currentCPUTime2 - currentCPUTime1;
        		//System.out.println(child +" subclassof "+parent +" with "+ isSubsumedBy +" " +stTime +" ms");             	
    ...
    protected Hierarchy<AtomicConcept> buildHierarchy(Relation<AtomicConcept> hierarchyRelation) {
        ...
        Set<AtomicConcept> remainingC = null;
    	remainingC = new HashSet(m_elements);

        if(useTech){
        	//Set<AtomicConcept> notCheckC = new HashSet(m_elements);
        	//notCheckC.removeAll(m_reAD.getCanS());
        	/*for(AtomicConcept ele:notCheckC){
        		m_possibleSubsumptions.getSuccessors(ele).clear();
        	}*/
        	remainingC.retainAll(m_reAD.getCanS());
        	remainingC.add(m_bottomElement);
        	remainingC.add(m_topElement);

        }
        
        
       
        for (AtomicConcept element : remainingC) {
        	 if(useTech){
             	m_reAD.addCheckedCanS();
             }
    ...
    protected Hierarchy<AtomicConcept> buildTransitivelyReducedHierarchy(Graph<AtomicConcept> knownSubsumptions,Set<AtomicConcept> elements) {
        ...
        //record the inferred results
        /*if(classificationFinished) {
            m_reAD.addAllSubsumers(allSubsumers);
        }*/
