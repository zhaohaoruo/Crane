...
public class DeterministicClassification {
    ...
    public DeterministicClassification(Tableau tableau,ClassificationProgressMonitor progressMonitor,AtomicConcept topElement,AtomicConcept bottomElement,Set<AtomicConcept> elements, ReADSetting reAD) {
        m_tableau=tableau;
        ...
        if(reAD!=null){
        	 m_reAD = reAD;
             //m_reAD.initialize(ReADType.NonDETER);
    ...
    public Hierarchy<AtomicConcept> classify() {
        ...
        Set<AtomicConcept> remainingC = new HashSet(m_elements);
        if(useTech){      	
        ...
        for (AtomicConcept element : remainingC) {
        	//if(remainingC.contains(element)){//begin to avoid SAT for elements
